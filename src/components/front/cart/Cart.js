    ...
    return (
        <div className="cart-items container-sm">
            ...
            <div>
                {cartitems.length >= 1 && (
                    <center>
                        <button className="btn btn-danger m-5" onClick={handleClearCart}>Clear Cart</button>
                    </center>
                )}
            </div>
            {cartitems.length === 0 && (
                <div className='cart-empty text-center' style={{color:"white"}}>Your cart is empty</div>
            )}
                    ...
                    <tbody>
                        {cartitems.map((item) => (
                            <tr key={item.id}>
                                ...
                                <td>
                                    <button className="btn btn-light in" onClick={() => handleAddProduct(item)}>+</button>
                                    <button className="btn btn-light in" onClick={() => handleRemoveProduct(item)}>-</button>
                                </td>
                            </tr>
                        ))}
                        <tr>
